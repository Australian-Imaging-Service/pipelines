...
def test_release_gating_permissions_and_queue(jobs):
    assert (
        jobs["build_and_deploy"]["if"]
        == "needs.discover-specs.outputs.has-builds == 'true'"
    )
    publisher = jobs["publish-catalogue"]
    assert publisher["needs"] == ["discover-specs", "build_and_deploy"]
    ...
    for required in (
        ...
        "needs.build_and_deploy.result == 'success' || needs.build_and_deploy.result == 'skipped'",
    ):
    ...
    for name in (
        ...
        condition = step(jobs["build_and_deploy"], name)["if"]
    ...
    assert jobs["build_and_deploy"]["permissions"]["contents"] == "read"
    assert (
...
def test_inventory_is_shared_with_builds_and_publication(jobs):
    ...
    for job, name in (
        ("build_and_deploy", "Download release inventory"),
...
def test_matrix_artifacts_feed_publication(jobs):
    assert step(jobs["build_and_deploy"], "Upload immutable release entry")["with"][
        "name"
